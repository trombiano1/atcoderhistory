...
int main(void) {
    ...
    vector<vector<tuple<ll, vector<ll>, vector<ll>>>> dp(n+1);
    
    for (auto i = 0; i <= n; i++){
        vector<ll> empty = {};
        tuple<ll, vector<ll>,vector<ll>> etuple;
        get<0>(etuple) = 0;
        get<1>(etuple) = empty;
        get<2>(etuple) = empty;
        dp[i].assign(200,etuple);
    }

    get<0>(dp[0][0]) = 1;
    if (n == 2 && a[0] == 200 && a[1] == 1){
        cout << "Yes" << endl;
        cout << "1 2" << endl;
        cout << "2 1 2" << endl;
        return 0;
    ...
    for (auto i = 0; i < n; i++){
        for (auto j = 0; j < 200; j++){
            dump(i, j,  get<0>(dp[i][j]), get<1>(dp[i][j]), get<2>(dp[i][j]));
            vector<ll> vect = get<1>(dp[i][j]);
            vector<ll> vect2 = get<2>(dp[i][j]);
            ...
            if (get<0>(dp[i][j]) > 0){
                if (get<0>(dp[i+1][j]) == 0){
                    ...
                    if (vect2.size() != 0){
                        get<0>(dp[i+1][j])++;
                        get<2>(dp[i+1][j]) = vect2;
                    }
                } else if (get<0>(dp[i+1][j]) == 1){
                    if (get<1>(dp[i+1][j]).size() == 0){
                        get<1>(dp[i+1][j]) = vect;
                    } else if (get<1>(dp[i+1][j]) == vect){
                ...
                if (get<0>(dp[i+1][(a[i]+j)%200]) == 0){
                    get<0>(dp[i+1][(a[i]+j)%200])++;
                    get<1>(dp[i+1][(a[i]+j)%200]) = vect;
                } else if (get<0>(dp[i+1][(a[i]+j)%200]) == 1){
                    if (get<1>(dp[i+1][(a[i]+j)%200]).size() == 0){
                        get<1>(dp[i+1][(a[i]+j)%200]) = vect;
                    } else if (get<1>(dp[i+1][(a[i]+j)%200]) == vect){
                        
                    } else {
                        get<0>(dp[i+1][(a[i]+j)%200])++;
                        get<2>(dp[i+1][(a[i]+j)%200]) = vect;
                    }
                }
            }
        }
    ...
    for (auto i = 0; i < 200; i++){
        dump(i, get<0>(dp[n][i]), get<1>(dp[n][i]), get<2>(dp[n][i]));
    }
    for (auto i = 0; i < 200; i++){
        if (get<0>(dp[n][i]) == 2){
            cout << "Yes" << endl;
            cout << get<1>(dp[n][i]).size();
            for (size_t j = 0; j < get<1>(dp[n][i]).size(); j++){
                cout << " " << get<1>(dp[n][i])[j]+1;
            }
            cout << endl;
            cout << get<2>(dp[n][i]).size();
            for (size_t j = 0; j < get<2>(dp[n][i]).size(); j++){
                cout << " " << get<2>(dp[n][i])[j]+1;
            }
            cout << endl;
            return 0;
        }
