...
int main(void) {
    ...
    vector<ll> d(m);
    cin >> a >> b >> c >> d;
    vector<pair<ll,ll>> cho(n);
    vector<pair<ll, ll>> box(m);
    for (auto i = 0; i < n; i++){
        cho[i] = make_pair(a[i],b[i]);
    }
    for (auto i = 0; i < m; i++){
        box[i] = make_pair(c[i], d[i]);
    }
    sort(ALL(cho), greater<pair<ll,ll>>());
    sort(ALL(box), greater<pair<ll,ll>>());
    dump(cho, box);
    vector<bool> used(m, false);
    for (auto i = 0; i < n; i++){
        for (auto j = 0; j < n; j++){
            if (!used[j] && cho[i].first < box[j].first && cho[i].second < box[j].second){
                used[j] = true;
                break;
            }
        }
