...
int main(void) {
    ...
    dump(a, b);
    SegTree<long long> a_diff(n, 0);
    for (int i = 1; i < n; i++){
        a_diff.update(i, abs(a[i]-a[i-1]));
    }
    dump(a_diff);
    SegTree<long long> b_diff(n, 0);
    for (int i = 1; i < n; i++){
        b_diff.update(i, abs(b[i]-b[i-1]));
    }
    dump(b_diff);
    for (int i = 0; i < q; i++){
        int h1, h2, w1, w2;
        ...
        cout << gcd(a[h1]+b[w1], gcd(a_diff.query(h1+1, h2+1), b_diff.query(w1+1, w2+1))) << endl;
    }
    return 0;
}