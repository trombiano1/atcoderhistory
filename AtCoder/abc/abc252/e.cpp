...
int main(void) {
    ...
    vector<vector<tuple<ll,ll,ll>>> G(n); // {cost, to}
    ...
    ll start = 0; // start point
    dist[start] = {0, -INF};
    pque.push({0, start,-INF});
    
    while (!pque.empty()) {
        tuple<ll,ll,ll> p = pque.top();
        pque.pop();
        ll d = get<0>(p); // dist (minなら確定)
        ll v = get<1>(p); // point name
        ll e = get<2>(p); // edge name

        if (dist[v].first < d) continue; // 最短以外は無視 else vは確定
        if (e != -INF){
            dump(v, e);
        }
        ...
        for (auto edge : G[v]) { // vに繋がってる点が更新できれば候補に入れる
            ...
            ll nv = get<1>(edge);
            ll ne = get<2>(edge);
            if (dist[nv].first > dist[v].first + c) {
                dist[nv] = {dist[v].first + c, ne};
                pque.push({dist[nv].first, nv, ne});
            }
        }
    }
    ...
    for (auto i = 0; i < n; i++){
        if (dist[i].second != -INF) cout << dist[i].second+1 << " ";
    }
    cout << endl;
    return 0;
}