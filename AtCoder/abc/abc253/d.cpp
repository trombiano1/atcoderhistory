...
int main(void) {
    ...
    ll all_sum = n * (n+1) / 2;
    ll a_num = n/a;
    ll a_sum = a * a_num * (a_num + 1) / 2;
    ll b_num = n/b;
    ll b_sum = b * b_num * (b_num + 1) / 2;
    ll lcm_num = n/lcm(a,b);
    ll lcm_sum = lcm(a,b) * lcm_num * (lcm_num + 1) / 2;
    dump(all_sum, a_sum, b_sum, lcm_sum);
    cout << all_sum - a_sum - b_sum + lcm_sum << endl;
    return 0;
}