...
int main(void) {
    ...
    for (auto i = 0; i < n; i++){
        if (s[i] == 'L'){
            mp[coord[i].second].push_back(make_pair(-1,coord[i].first));
        } else {
            mp[coord[i].second].push_back(make_pair(1,coord[i].first));
        }
    }
    for (auto y_m : mp){
        if (y_m.second.size() == 1) continue;
        dump(y_m.first, y_m.second);
        ll right_min = INF;
        ll left_max = -INF;
        for (auto hor: y_m.second){
            if (hor.first == 1 && right_min > hor.second){
                right_min = hor.second;
            }
            if (hor.first == -1 && left_max < hor.second){
                left_max = hor.second;
            }
        }
        dump(right_min, left_max);
