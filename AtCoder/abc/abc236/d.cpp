...
ll dfs(vector<bool> used, ll res){
    ll max = 0;
    vector<ll> unused;
    for (size_t i = 0; i < used.size(); i++) if (!used[i]) unused.push_back(i);
    if (unused.size() == 0) {
    ...
    for (size_t i = 1; i < unused.size(); i++){
        vector<bool> current = used;
        current[first] = true;
        current[unused[i]] = true;
        dfs(current, res ^ a[first][unused[i]]);
    }
    return max;
}
...
int main(void) {
    ...
    dump(a);
    vector<bool> all_unused(2*n, false);
    dfs(all_unused, 0);
