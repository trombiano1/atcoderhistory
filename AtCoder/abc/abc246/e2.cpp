...
int main(void) {
    ...
    while (!que.empty()) {
        vector<ll> v = que.front(); // キューから先頭頂点を取り出す
        dump(v);
        que.pop();
        // v から辿れる頂点をすべて調べる
        for (auto dir : dirs){
            for (auto d = 1;;d++){
                ll new_x = v[0] + dir.first * d;
                ll new_y = v[1] + dir.second * d;
                dump(new_x,new_y);
                if (new_x >= n || new_y >= n) break;
                if (new_x < 0 || new_y < 0) break;
                if (board[new_x][new_y]) break;
                if (dist[new_x][new_y] != -1) continue; // すでに発見済みの頂点は探索しない
