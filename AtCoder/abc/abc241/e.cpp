...
int main(void) {
    ...
    dump(loop_last, x_last, a[x_last%n], x_last%n);
    ll x_first = 0;
    ll loop_first = 0;
    for (;;) {
        if (x_first%n == x_last%n){
            break;
        }
        x_first += a[x_first%n];
        loop_first++;
    }
    dump(loop_first, x_first);
    dump(x_last - x_first, loop_last - loop_first);
    k -= loop_first;
    ll res = x_first;
    res += (x_last - x_first)*(k/(loop_last-loop_first));
    k %= loop_last - loop_first;
    for (auto i = 0; i < k; i++){
        res += a[x_last%n];
        x_last += a[x_last%n];
    }
