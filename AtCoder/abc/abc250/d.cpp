...
int main(void) {
    ...
    cin >> n;
    vector<bool> primes;
    ll max_q = ceil(cbrt((double)n));
    primes = Eratosthenes(max_q);
    vector<ll> primecount(max_q);
    ll counter = 0;
    for (auto i = 1; i < max_q; i++){
        if (primes[i]) counter++;
    ...
    for (ll i = 3; i * i * i < n; i++){
        if (is_prime(i)){
            double q3 = i*i*i;
            res += primecount[min((ll)((double)n/q3), (ll)(i-1))];
            // dump(primecount[min((ll)((double)n/q3), (ll)i)]);
        }
    }
    cout << res << endl;
    return 0;
