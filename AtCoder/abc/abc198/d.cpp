...
ll tp;
vector<vector<ll>> n(3);
bool done = false;

void dfs(vector<ll> decided, vector<bool> used){
    if (done) return;
    if ((ll)decided.size() == tp){
        // dump(decided, used);
        ll n1 = 0, n2 = 0, n3 = 0;
        if (decided[n[0][0]] == 0) return;
        if (decided[n[1][0]] == 0) return;
        if (decided[n[2][0]] == 0) return;
    ...
    vector<ll> unused;
    for (auto i = 0; i < 10; i++) if (!used[i]) unused.push_back(i);
    for (auto nw : unused){
        vector<ll> new_decided = decided;
        vector<bool> new_used = used;
        new_decided.push_back(nw);
        new_used[nw] = true;
        dfs(new_decided,new_used);
    }
}
