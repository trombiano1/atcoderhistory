...
int main(void) {
    ...
    for (auto i = 1; i < n; i++){
        ll ltr = s[i] - 'A';
        for (ll bit = 0; bit < (1<<10); ++bit){
            for (ll alp = 0; alp < 10; ++alp){

                dp[i][bit][alp] += dp[i-1][bit][alp]; //使わない場合
                //使う場合
                  //最後と同じ文字
                if (alp == ltr){
                    dp[i][bit][alp] += dp[i-1][bit][alp];
                    if (bit == 0){
                        dp[i][(1<<ltr)][alp] += 1;
                    }
                } else {
                    if (!(bit & (1<<ltr))){
                        ll nbit = bit;
                        nbit |= (1<<ltr);
                        dp[i][nbit][ltr] += dp[i-1][bit][alp];
                    }
                }
                dp[i][bit][alp] %= 998244353;
            }
