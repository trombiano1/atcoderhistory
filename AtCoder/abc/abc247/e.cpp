...
int main(void) {
    ...
    for (auto i = 0; i < n; i++){
        if (a[i] < y || a[i] > x){
            vects.push_back(currvect);
            currvect = {};
        } else {
            currvect.push_back(a[i]);
        }
    }
    vects.push_back(currvect);
    dump(vects);
    ...
    for (auto vect : vects){
        ...
        for (size_t i = 0; i < vect.size(); i++){
            if (vect[i] == x){
                ...
                currv.push_back(vect[i]);
            }
        }
        v.push_back(currv);
        ll nomax = 0;
        for (size_t i = 0; i < v.size(); i++){
            nomax += (v[i].size())*(v[i].size()+1)/2;
        }
        ...
        ll noboth = 0;
        for (size_t i = 0; i < v.size(); i++){
            noboth += (v[i].size())*(v[i].size()+1)/2;
        }
        
        dump(v, nomax, nomin, noboth);
        res += (vect.size()) * (vect.size()+1) / 2 - nomax - nomin + noboth;
    }
    cout << res << endl;
    return 0;
}