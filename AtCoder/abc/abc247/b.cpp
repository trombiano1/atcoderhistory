...
int main(void) {
    ...
    dump(names);
    for (auto i = 0; i < n; i++){
        string s = names[i].first;
        string t = names[i].second;
        bool s_okay_flag = true, t_okay_flag = true;
        for (auto j = 0; j < n; j++){
            if (i == j) continue;
            if (s == names[j].first || s == names[j].second) s_okay_flag = false;
            if (t == names[j].first || t == names[j].second) t_okay_flag = false;
            if (!s_okay_flag && !t_okay_flag){
                cout << "No" << endl;
                return 0;
            }
        }
