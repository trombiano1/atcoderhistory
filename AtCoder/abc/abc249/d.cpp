...
int main(void) {
    ...
    ll res = 0;
    for (auto i = 0; i < n; i++){
        vector<ll> divisors = enum_divisors(a[i]);
        for (size_t j = 0; j < divisors.size(); j++){
            if (nums[divisors[j]] > 0 && nums[a[i] / divisors[j]] > 0){
                if (divisors[j] == a[i]/divisors[j]){
                    res += nums[divisors[j]] * nums[a[i] / divisors[j]];
                } else {
                    res += 2 * nums[divisors[j]] * nums[a[i] / divisors[j]];
                }
            }
        }
    }
